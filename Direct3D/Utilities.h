...
namespace Utilities
{
	...
	class DebugTri
	{
		BoundingBox box;
		Plane plane;
		LineModel* triModel = nullptr;
		LineModel* boxModel = nullptr;
		DirectX::XMMATRIX trans = DirectX::XMMatrixIdentity();
		
	public:
		DebugTri() = default;
		~DebugTri();
		DebugTri(Plane& p, BoundingBox &b);
		LineModel* GetTriModel();
		LineModel* GetBoxModel();
		void Rasterize(ShaderFactory::Shader* shader, ID3D11ShaderResourceView* t);
	};
