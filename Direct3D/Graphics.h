...
public:
	...
	HRESULT EndScene();
	HRESULT RenderTerrain(class Terrain* terrain, ShaderFactory::Shader* shader);
	HRESULT RenderModel(class WavefrontModel& model, const DirectX::XMMATRIX& matrix, ShaderFactory::Shader* shader);
	HRESULT RenderModel(class Model2& model, const DirectX::XMMATRIX& matrix, ShaderFactory::Shader* shader);
	HRESULT RenderFoliage(class FoliageClass* foliage, ShaderFactory::Shader* shader);
	///////////
	// Direct 2D
	///////////
