...
LineShader::LineShader(ShaderFactory::InitShaderDesc desc)
{
	ID3DBlob* vblob;
	ID3DBlob* pblob;
	...
	assert(m_sampler);
	ShaderFactory::InputLayoutDesc::CreateLayout(ShaderFactory::InputLayoutDesc::PositionColor32, numElements, vblob, &m_inputLayout);
...
HRESULT LineShader::Render(ID3D11DeviceContext * context, ID3D11Buffer* VBuffer, ID3D11Buffer* IBuffer,
