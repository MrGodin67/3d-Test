...
bool Direct3DWindow::ProcessMessage()
{
	...
	UpdateEventArgs updateEventArgs(timer.Time(), totalTime);
	RenderEventArgs renderEventArgs(timer.Time(), totalTime);
	cam.FrameMove(timer.Time());
...
LRESULT CALLBACK Direct3DWindow::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	...
	cam.HandleMessages(hwnd, umsg, wparam, lparam);
	switch (umsg)
	{
	case WM_KEYDOWN:
	{
