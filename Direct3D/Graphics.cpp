	...
	Direct3D((UINT)screenWidth,(UINT)screenHeight,hwnd,v_sync,fullscreen)
{
	...
	MenuBase::SetViewPortCenter(resX,resY);
...
HRESULT Graphics::RenderModel(WavefrontModel & model, const DirectX::XMMATRIX & matrix, ShaderFactory::Shader * shader)
{
	m_pImmediateContext->UpdateSubresource(ShaderFactory::ConstMatrixBuffers[ShaderFactory::CB_object], 0, nullptr, &XMMatrixTranspose(matrix), 0, 0);
	std::vector<ID3D11ShaderResourceView*> tex;
	HRESULT hr;
	//EnableAlphaBlending();
	for (int c = 0; c < model.MeshCount(); c++)
	{
		tex.clear();
		tex.push_back(model.GetMesh(c)->GetTexture(0));
		hr = shader->Render(m_pImmediateContext.Get(),
...
HRESULT Graphics::RenderFoliage(FoliageClass * foliage, ShaderFactory::Shader * shader)
{
	std::vector<ID3D11ShaderResourceView*> tex;
	tex.push_back(foliage->GetTexture());
	HRESULT hr = S_OK;
	shader->SetVisibleCount(foliage->GetVisibleCount());
	hr = shader->Render(m_pImmediateContext.Get(), foliage->GetVertexBuffer(), foliage->GetInstanceBuffer(),
		foliage->VertexStructSize(), foliage->InstanceStructSize(), tex);
	assert(hr == S_OK);
	return hr;
}
// Direct 2D
